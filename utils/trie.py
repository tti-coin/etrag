...
class PredictTrieNode(object):
    def __init__(self, prob=None, rel=None):
        self.prob = prob
        self.rel = rel
        self.next_token = {}
...
class PredictTrie(object):
    def __init__(self, tokenizer, force_end=True):
        self.tokenizer = tokenizer
    ...
    def predict(self, subj, obj, raw_templates, inputs, model, keep_prompt=False, return_retrieval=False):
        templates = {}
        ...
        while queue:
            ...
            if node.children_num > 0:
                ...
                if node.children_num == 1:
                    ...
                    generate_output = self.get_predict_logit(model, inputs, decoder_inputs, return_logits=False)
                    logits = generate_output.logits[0, -1, :].cpu()

                    if hasattr(generate_output, "prompts"):
                        prompts = generate_output.prompts
                    if retrieval_result is None and hasattr(generate_output, "retriever_output"):
                        retrieval_result = generate_output.retriever_output
...
class BatchPredictTrie(object):
    def __init__(self, tokenizer, templates):
        self.tokenizer = tokenizer
        self.templates = templates
        self.trie = self.build_tree(templates)
    ...
    def predict(self, subj, obj, sentence, model, device):
        encoder_inputs = self.tokenizer.batch_encode_plus(
            [sentence] * len(self.path_set), return_tensors="pt", max_length=256, padding="max_length", truncation=True
        ).to(device)
        decoder_sents = []
        ...
        decoder_inputs = self.tokenizer.batch_encode_plus(
            decoder_sents, return_tensors="pt", max_length=64, padding="max_length", truncation=True
        ).to(device)
